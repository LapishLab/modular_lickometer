...
class ModeDefinition:
	def __init__(self, mode, start_on, stop_on):
		self.mode = mode
...
class ModeHandler:
	...
	def _create_watch_tasks(self):
		...
		for signal in all_signals:
			if not self._contains(already_watched, signal):
				...
				watch_tasks.append(
					asyncio.create_task(self._watch(signal))
				)

		return watch_tasks

	async def _watch(self, signal):
		while True:
			await signal.wait()
			self._route(signal)

	def _route(self, signal):
	...
	def _contains(self, signals, target):
		return any(target is signal for signal in signals)

	async def wait_for_mode(self):
